...
(function(window, undefined) {
		...
		data : function(key, value) {
			...
			return jQuery.access(this, function(value) {
				...
				if (value === undefined) {
					...
					return elem ? dataAttr(elem, key, jQuery.data(elem, key))
							: null;
				}
	...
	jQuery
			.extend({
				...
				attr : function(elem, name, value) {
					...
					} else if (hooks && notxml && "get" in hooks
	...
	boolHook = {
		get : function(elem, name) {
			...
			ruseDefault.test(name) ? elem[jQuery.camelCase("default-" + name)]
					...
					// fetch an attribute node for properties not recognized as
					// boolean
					elem.getAttributeNode(name);

			return detail && detail.value !== false ? name.toLowerCase()
					: undefined;
		},
	...
	(function(window, undefined) {
		...
		setDocument = Sizzle.setDocument = function(node) {
			...
			// Tag
			Expr.find["TAG"] = support.tagNameNoComments ? function(tag,
					context) {
			...
			sortOrder = docElem.compareDocumentPosition ? function(a, b) {
				...
				if ((compare = b.compareDocumentPosition
					...
					if (compare & 1 || a.parentNode
							&& a.parentNode.nodeType === 11) {
		...
		Sizzle.matchesSelector = function(elem, expr) {
			...
			// check
			if (support.matchesSelector && !documentIsXML
		...
		Sizzle.attr = function(elem, name) {
			...
			return ((val = elem.getAttributeNode(name)) || elem
					.getAttribute(name))
					&& elem[name] === true ? name
		...
		function siblingCheck(a, b) {
			var cur = b && a, diff = cur && (~b.sourceIndex || MAX_NEGATIVE)
		...
		Expr = Sizzle.selectors = {
			...
			filter : {
				...
				"TAG" : function(nodeName) {
					...
					nodeName = nodeName.replace(runescape, funescape)
							.toLowerCase();
					return function(elem) {
						return elem.nodeName
			...
			pseudos : {
				...
				"lang" : markFunction(function(lang) {
					...
					return function(elem) {
						var elemLang;
						do {
							if ((elemLang = documentIsXML ? elem
		...
		function select(selector, context, results, seed) {
			...
			// selector above
			compile(selector, match)(seed, context, documentIsXML, results,
			...
			.extend({
				...
				remove : function(selector, keepData) {
					...
					for (; (elem = this[i]) != null; i++) {
						if (!selector
								|| jQuery.filter(selector, [ elem ]).length > 0) {
				...
				domManip : function(args, table, callback) {
					...
					if (isFunction
						...
						return this.each(function(index) {
							...
							if (isFunction) {
								args[0] = value.call(this, index, table ? self
										.html() : undefined);
							}
